...
def get_y_values(target: Target, steps: int):
    y_values = []
    max_y = (steps - 1) // 2 # at this point, we are at zero
    for y in range(max_y, target[2]-1, -1):
        # these are the ys to check
        final_y = get_y_value(y, steps)
        if target[2] <= final_y <= target[3]:
            y_values.append(y)
    return y_values
